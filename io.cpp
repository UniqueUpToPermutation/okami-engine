...
namespace okami {
    ...
    protected:
        OnResourceLoadedSignal<Texture> LoadResource(LoadResourceSignal<Texture>&& msg) override {
            // Get the extension and decide loading method
            auto ext = msg.m_path.extension().string();
            if (ext == ".ktx2" || ext == ".KTX2") {
                return OnResourceLoadedSignal<Texture>{
    ...
    protected:
        OnResourceLoadedSignal<Geometry> LoadResource(LoadResourceSignal<Geometry>&& msg) override {
            // Get the extension and decide loading method
            auto ext = msg.m_path.extension().string();
            if (ext == ".glb" || ext == ".GLB" || ext == ".gltf" || ext == ".GLTF") {
                return OnResourceLoadedSignal<Geometry>{
